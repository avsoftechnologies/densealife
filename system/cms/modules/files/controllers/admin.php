...
class Admin extends Admin_Controller {
	...
	public function new_folder()
	{
		...
		$name = $this->input->post('name');
                ...
                $result = array();
                if(!empty($event)){
                    $result = Files::create_album($parent_id, $name, $event);
                }else{
	...
	public function folder_contents()
	{
		...
		ob_end_clean();
		echo json_encode(Files::folder_contents($parent, $event_id));
	}
	...
	public function delete_folder()
	{
		...
		if ($id = $this->input->post('folder_id'))
		{
			...
			$result['status'] AND Events::trigger('file_folder_deleted', $id, 'json');

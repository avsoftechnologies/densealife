...
function load_partial($file, $data = array())
{
    $ext = pathinfo($file, PATHINFO_EXTENSION) ? '' : '.php';

    if ( file_exists(ci()->template->get_views_path() . 'partials/' . $file . $ext) ) {
        // look in the theme for overloaded views
    ...
    return ci()->load->_ci_load(array( '_ci_view' => $file, '_ci_return' => true ));
}
function p($data)
{
    echo '<pre>'; 

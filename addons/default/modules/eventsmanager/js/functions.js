jQuery(function($) {
   ...
   $(document).keydown(function(event){
    ...
    $('.context-menu-source, .button-menu-source').on('click', '[data-menu]', function(e) {
        ...
        switch (menu) {
            ...
            case 'create-thumbnail':
                pyro.files.create_thumbnail();
                break;
             
            case 'make_public':
                pyro.files.modify_access_level('public');
                break;
             
            case 'make_private':
                pyro.files.modify_access_level('private');
                break;
                
            case 'make_protected':
                pyro.files.modify_access_level('protected');
                break;
        }
    });
    
    // modifying access level of the files
    
    pyro.files.modify_access_level = function(access){
        var fileId = pyro.files.$last_r_click.data('id');
        $.post(
                SITE_URL + 'admin/eventsmanager/change_access_level',
                {
                    file_id: fileId, 
                    access_level: access
                }, 
    ...
    pyro.files.new_folder = function(parent, name) {
        ...
        post_data = {parent: parent, name: name, event: event_id};
    ...
    pyro.files.folder_contents = function(folder_id) {
        ...
        post_data = {parent: folder_id, event: event_id};
            ...
            if (results.status) {
                ...
                content_interval = window.setInterval(function() {
                    ...
                    $folders_center.append(
                            '<li class="files-tooltip ' + item.el_type + ' ' + (item.el_type === 'file' ? 'type-' + item.type : '') + '" data-id="' + item.id + '" data-name="' + item.name + '"  data-filename="' + item.filename + '">' +
                            li_content +
                            '</li>'
                            );
    ...
    pyro.files.delete_item = function(current_level) {
        ...
        items.each(function(index, item) {
            ...
            $.post(SITE_URL + 'admin/files/delete_' + type, post_data, function(results) {
                if (results.status) {
                    // delete locally
    ...
    pyro.files.details = function() {
        ...
        if ($item) {
            if ($item.id) {
                $item_details.find('.id').html($item.id).parent().show();
            }
            if ($item.name) {
                $item_details.find('.name').html($item.name).parent().show();
            }
            if ($item.slug) {
                $item_details.find('.slug').html($item.slug).parent().show();
            }
            if ($item.path) {
                $item_details.find('.path').val($item.path).parent().show();
            }
            if ($item.formatted_date) {
                $item_details.find('.added').html($item.formatted_date).parent().show();
            }
            if ($item.width > 0) {
                $item_details.find('.width').html($item.width + 'px').parent().show();
            }
            if ($item.height > 0) {
                $item_details.find('.height').html($item.height + 'px').parent().show();
            }
            if ($item.filesize) {
                $item_details.find('.filesize').html(($item.filesize < 1000 ? $item.filesize + 'Kb' : $item.filesize / 1000 + 'MB')).parent().show();
            }
            if ($item.download_count) {
                $item_details.find('.download_count').html($item.download_count).parent().show();
            }
            if ($item.filename) {
                $item_details.find('.filename').html($item.filename).parent().show();
            }
            if (type === 'file') {
                $item_details.find('.description').val($item.description).parent().show();
            }
            if (type === 'file') {
                $item_details.find('#keyword_input').val($item.keywords).parent().show();
            }
            if (type === 'file') {
                $item_details.find('.show-data').show();
            }
            if ($item.type === 'i') {
                $item_details.find('.alt_attribute').val($item.alt_attribute).parent().show();
            }
    ...
    pyro.files.create_thumbnail = function() {
         ...
         var timer, location,
                ...
                $filename = pyro.files.$last_r_click.data('filename').length > 0 ? pyro.files.$last_r_click.data('filename') : pyro.files.current_level; // end var

        $('div#create-thumbnail').find('img').attr('src', 'uploads/default/files/'+$filename+"?"+Math.floor(Math.random()*10000));
        
            $.colorbox({
                scrolling: false,
                inline: true,
                href: 'div#create-thumbnail',
                opacity: 0,
                height: 650,
                width:700
            });
...
//        $item_details.find('img').attr('width','630').attr('height','500');
           // $.colorbox.resize();
            
                // save on click, then close the modal
            $('div#create-thumbnail').find('.buttons').on('click', 'button', function() {
                if (type === 'file' && checkCoords()) {    
                    
                    x = $('#thumb_x').val(); 
                    y = $('#thumb_y').val();
                    w = $('#thumb_w').val(); 
                    h = $('#thumb_h').val();
                    disp_w =  $('div#create-thumbnail').find('img').css('width').replace('px','');
                    disp_h =  $('div#create-thumbnail').find('img').css('height').replace('px','');
                    
                    $.post(SITE_URL + 'admin/eventsmanager/save_thumbnail', 
                        {
                            disp_w: disp_w, 
                            disp_h: disp_h,
                            x: x, 
    ...
    pyro.files.save_thumbnail = function(x, y, w, h , pictureid){

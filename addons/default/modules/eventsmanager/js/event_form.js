$(function() {
    $('.drpdwn_category_id').chosen().change(function() {
        
        id = this.id; 
        div  = $('#'+id+'_chzn');
        $.post(
                'admin/eventsmanager/get_sub_categories', 
                {
                    cat_id: $(this).val()
                }, 
                function(subcategories) {
                    ...
                    $('.drpdwn_sub_category_id').trigger("liszt:updated");
...
//		});
    ...
    $(document.getElementById('content-tab')).find('ul').find('li').first().find('a').colorbox({
        srollable: false,
        innerWidth: 600,
        innerHeight: 280,
        href: SITE_URL + 'admin/eventsmanager/categories/create_ajax',
        onComplete: function() {
            $.colorbox.resize();
            ...
            $form_categories.live('submit', function(e) {
                ...
                $.ajax({
                    url: SITE_URL + 'admin/eventsmanager/categories/create_ajax',
                    type: "POST",
                    data: form_data,
                    success: function(obj) {
                        ...
                        if (obj.status == 'ok') {
                            ...
                            $(document.getElementById('content-tab')).find('li').first().find('span').html(obj.title);

...
class Admin_Categories extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $total_rows = $this->event_categories_m->count_by(array( 'status' => '1' )) ;
        $pagination = create_pagination('admin/eventsmanager/categories/index', $total_rows, Settings::get('records_per_page'), 5) ;

        // Using this data, get the relevant results

        $categories = $this->event_categories_m->get_all_categories($pagination['limit'], $pagination['offset']) ;
    ...
    public function create()
    {
        ...
        if ( $this->form_validation->run() ) {
            ...
            redirect('admin/eventsmanager/categories') ;
        }
        ...
        $parent_categories = $this->event_categories_m
                ->order_by('title')
                ->get_many_by(array('status' => '1','parent_id' => 0 )) ;
    ...
    public function edit($id = 0)
    {
        ...
        // ID specified?
        $category or redirect('admin/eventsmanager/categories/index') ;

        $this->form_validation->set_rules('id', 'ID', 'trim|required|numeric') ;

        // Validate the results
        if ( $this->form_validation->run() ) {
            $this->event_categories_m->update($id, $this->input->post()) ? $this->session->set_flashdata('success', sprintf(lang('cat:edit_success'), $this->input->post('title'))) : $this->session->set_flashdata('error', lang('cat:edit_error')) ;

            // Fire an event. A Event category is being updated.
            Events::trigger('event_category_updated', $id) ;

            redirect('admin/eventsmanager/categories/index') ;
        }
    ...
    public function delete($id = 0)
    {
        ...
        if ( !empty($id_array) ) {
            ...
            foreach ( $id_array as $id ) {
                $allowed = true; 
                if ( $allowed = !$this->event_categories_m->is_category_associated($id) && $this->event_categories_m->delete($id) ) {
                    $deleted++ ;
                    $deleted_ids[] = $id ;
                } else {
                    if(!$allowed){
                        $this->session->set_flashdata('error', 'Category is associated with the event and can not be deleted') ;
                    }else{
